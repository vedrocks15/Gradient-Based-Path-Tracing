...
// Data structures to hold properties of light rays...
struct Vec {        
...
// Sphere is the fundamental element to represent objects in this scene...
struct Sphere {
    double rad;       // radius
...
// Helps in tracing a path while evaluating the rendering integral recursively
struct PathVert {
    Vec p; Vec n; int id;
};

// Holds all the vertices we go through...
struct Path {
    // Collection of all hit vertices
    PathVert verts[MAX_DEPTH];

    // Random numbers associated with the ray that creats the hit vertex
    double rnds[2*MAX_DEPTH];
    int vertCount;
    int x, y;
};

// Helper to check if we hit something or not...
...
bool generatePath(int x, int y, unsigned short *rng, Path &path) {  
    // For each (x,y) vertex location we store a random number....
    path.x = x; path.y = y;
    path.rnds[0] = erand48(rng); path.rnds[1] = erand48(rng); 

    // Sampling primary ray shot from the camera...
    Vec d = cx*( (path.rnds[0] + x)/width - .5) +
            cy*( (path.rnds[1] + y)/height - .5) + cam.d;  


    // Camera rays are pushed forward to start in interior (some origin calculation)
    ...
    for (int depth = 1; depth <= MAX_DEPTH; depth++) {
        ...
        // Hit Vertex location, normal & object id
        vert.p = ray.o + ray.d*t; vert.n = (vert.p - obj.p).norm(); vert.id = id;    
        // Store all details about hit vertex
        ...
        // End if max depth achieved & no light hit
        if (depth == MAX_DEPTH) return false;

        // Store x & y random numbers for each hit vertex
        path.rnds[2*depth] = erand48(rng); 
        path.rnds[2*depth+1] = erand48(rng);


        // Get New ray (hemispherical sampling)
...
bool shiftPath(int x, int y, const Path &basePath, Path &offsetPath, double &jacobian) {    
    ...
    for (int vertId = 0; vertId < basePath.vertCount; vertId++) {
        ...
        // Intersection point for the shifted ray....
        if (!intersect(ray, t, id)) return false;        
        const Sphere &obj = spheres[id]; 
        const Sphere &baseObj = spheres[basePath.verts[vertId].id];
        
        // If the surface type of the object hit by basepath does not match with og path : 
        if (obj.refl != baseObj.refl) return false;
        
        // Computing details of hit : 
        PathVert vert; 
        vert.p = ray.o + ray.d*t; vert.n = (vert.p - obj.p).norm(); vert.id = id;    
        offsetPath.verts[vertId] = vert; 
        
        // Reached all the possible vertices :
        if (vertId == basePath.vertCount - 1) break;

        // Diffuse condition for re joining to basePath
        if (obj.refl == DIFF && spheres[basePath.verts[vertId + 1].id].refl == DIFF) {
            
            // connect back to base path, jacobian = ratio of geometry term (handling the blocking case)
...
// path contribution in solid angle domain (handles the 3 cases of diffuse, specular, refraction)
Vec pathContrib(const Path &path) {   
    ...
    for (int vert = 0; vert < path.vertCount - 1; vert++) {
        ...
        // Dot product between normal of current vertex & outgoing ray       
        ...
        // Condition for various surfaces...
        if (obj.refl == DIFF) {
    ...
    // Final object emission multiplied....
...
int main(int argc, char *argv[]){
    ...
    // Defining number of rays to shoot per pixel
    int samps = argc==2 ? atoi(argv[1]) : 100; // # samples

    // width & height are pre-defined constants...

    // primal image & estimated gradient image...
    // each pixel has 4 neighbours therefore 4 gradient images...
...
#pragma omp parallel for schedule(dynamic, 1) // OpenMP
    ...
    for (int y=0; y<height; y++){                      
        ...
        for (unsigned short x=0, rng[3]={0,0,y*y*y}; x<width; x++) {  
            // primal, gradient estimates
            Vec r, rdx0, rdy0, rdx1, rdy1;

            // Going over each sample/pixel....
            for (int s=0; s<samps; s++){
                // Variables to hold og path, offset path & jacobian already
                Path path, oPath; double jacobian;        
                
                // start tracing... (path variable will hold the base path)
                if(generatePath(x, y, rng, path)) {                     
                    
                    // Computing probability & surface color contribution for rendering intergal
                    ...
                    if (prob > 0.0) {    
                        ...
                        // Shift mapping to offset path & jacobian      
            ...
            // Normal image contribution
            c[i]  = c[i] + r; 

            // all 4 neighbour pixels...

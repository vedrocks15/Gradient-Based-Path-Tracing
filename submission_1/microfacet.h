...
inline Real metal_smith_masking_gtr2(const Vector3 &v_local, Real roughness, Real alpha_x, Real alpha_y) {
    Real alpha = roughness * roughness;
    Real a2 = alpha * alpha;
    Vector3 v2 = v_local * v_local;
...
inline Vector3 sample_visible_normals_custom(const Vector3 &local_dir_in, Real alpha, const Vector2 &rnd_param, Real alpha_x, Real alpha_y) {
    // The incoming direction is in the "ellipsodial configuration" in Heitz's paper
    if (local_dir_in.z < 0) {
        // Ensure the input is on top of the surface.
        return -sample_visible_normals_custom(-local_dir_in, alpha, rnd_param, alpha_x, alpha_y);
    }
...
inline Vector3 sample_clearcoat(const Vector3 &local_dir_in, Real alpha, const Vector2 &rnd_param) {
    
    Real u_0 = rnd_param.x;
    Real u_1 = rnd_param.y;

    Real h_azim = 2*c_PI*u_1;
    Real sin_h_elevation = pow(((pow(alpha*alpha,1-u_0)  - alpha*alpha)/(1-alpha*alpha)),0.5);
    Real cos_h_elevation = pow(((1 - pow(alpha*alpha,1-u_0))/(1-alpha*alpha)),0.5);
    Real h_l_x = sin_h_elevation*cos(h_azim);
    Real h_l_y = sin_h_elevation*sin(h_azim);
    Real h_l_z = cos_h_elevation;

    return normalize(Vector3{h_l_x, h_l_y, h_l_z});
}
